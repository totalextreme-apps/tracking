...
const getTrailerUrl = (movieData: any) => {
...
function MovieDetailReactionSection({ collectionItemId, userId }: { collectionItemId: string; userId?: string }) {
...
export default function MovieDetailScreen() {
    ...
    const [editingNoteForFormat, setEditingNoteForFormat] = useState<string | null>(null);
    ...
    useEffect(() => {
        if (isReadOnly || !movie || !movie.title || movieItems.length === 0) return;

        movieItems.forEach(async (item: any) => {
            ...
            try {
                ...
                console.error(`Failed to auto-lookup price for ${movie.title} (${item.format}):`, err);
            }
        });
                        ...
                        <View className="flex-1 ml-4 pt-1">
                            ...
                            <View className="flex-row items-center gap-2 mt-2.5 flex-wrap">
                                {displayMovie.tmdb_id && (
                                    <Pressable
                                        onPress={() => Linking.openURL(`https://letterboxd.com/tmdb/${displayMovie.tmdb_id}`)}
                                ...
                                {(() => {
                                    const trailerUrl = getTrailerUrl(tmdbMovie || displayMovie);
